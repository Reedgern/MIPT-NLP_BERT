...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    sf_input = torch.matmul(query, key.transpose(2,3)) + attention_mask
    softmax = torch.softmax(sf_input / self.attention_head_size**(1/2), 3)
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    layer1_res = self.add_norm(hidden_states, attention, self.attention_dense, 
                                    self.attention_dropout, self.attention_layer_norm)
    
    # feed forward
    
    forward = self.interm_af(self.interm_dense(layer1_res))

    # another add-norm layer
    
    res = self.add_norm(layer1_res, forward, self.out_dense, self.out_dropout, self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    pos_ids = torch.arange(seq_length, device=input_ids.device).repeat(input_shape[0], 1)
